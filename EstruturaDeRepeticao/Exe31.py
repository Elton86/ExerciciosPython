"""O Sr. Manoel Joaquim expandiu seus negócios para além dos negócios de 1,99 e agora possui uma loja de conveniências.
 ...
 valores referentes aos preços das mercadorias. Um valor zero deve ser informado pelo operador para indicar o final
...
cont = 1
while True:
    valor = float(input("Digite o valor do produto {}: ".format(cont)))
    if valor == -1:
        break
    else:
        compra["Produto " + str(cont)] = valor
        cont += 1
        soma += valor
for k, v in compra.items():
    print("{} - R${:.2f}".format(k, v))
